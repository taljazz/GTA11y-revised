...
namespace GrandTheftAccessibility
{
    class GTA11Y : Script
    {
            ...
            if (!keys_disabled)
            {
                ...
                if (e.KeyCode == Keys.NumPad0 && !keyState[0])
                {
                    ...
                    else
                    {
                        ...
                        {
                            Tolk.Speak($"Current location: Inside of a {Game.Player.Character.CurrentVehicle.DisplayName} at {World.GetStreetName(Game.Player.Character.Position)}.");
                        }
                    }
                }
        ...
        void setupSettings()
        {
            ...
            string json;
            string[] ids = { "announceHeadings", "announceZones", "announceTime", "altitudeIndicator", "targetPitchIndicator", "radioOff", "warpInsideVehicle", "onscreen", "speed", "godMode", "policeIgnore", "vehicleSpeed", "vehicleGodMode", "infiniteAmmo", "neverWanted", "superJump", "runFaster", "swimFaster", "exsplosiveAmmo", "fireAmmo", "explosiveMelee", "autoAim" };
